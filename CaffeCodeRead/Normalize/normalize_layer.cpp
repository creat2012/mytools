...
void NormalizeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* buffer_data = buffer_.mutable_cpu_data(); // buffer data ? 
  Dtype* norm_data = norm_.mutable_cpu_data();  // norm data ?
  // add eps to avoid overflow
  caffe_set<Dtype>(norm_.count(), Dtype(eps_), norm_data); // norm_data = eps for all
  ...
  int dim = bottom[0]->count() / num; // dim = channels * height * width
  int spatial_dim = bottom[0]->height() * bottom[0]->width(); // spatial_dim = height * width
  int channels = bottom[0]->channels();
  for (int n = 0; n < num; ++n) {
    //  buffer_data[i] = bottom_data[i] ^ 2
    // buffer_data (channels * width * height), bottom_data (channels * height * width)
    caffe_sqr<Dtype>(dim, bottom_data, buffer_data);
    if (across_spatial_) { // across_spatial 为是否整个 butter_data进行 normalize
      // norm_data[n] = (sum(buffer_data[i]) + eps) ^ 0.5
      // buffer_data (channels * height * width), norm_data (1 * (width * heigth))
      norm_data[n] = pow(caffe_cpu_asum<Dtype>(dim, buffer_data)+eps_,
                         Dtype(0.5));
      // top_data[i] = 1 / norm_data[n] * bottom_data
      caffe_cpu_scale<Dtype>(dim, Dtype(1.0 / norm_data[n]), bottom_data,
                             top_data);
    } else {
      // for channels 
      // ? ?  buffer_data (dim * 1)  sum_channel_multiplier ()
      // sum_channel_multiplier_.Reshape(1, channels, 1, 1);
      // CblasTrans is for transpose , so the buffer_data is the shape of (height * width) * channels
      // cblastrans is for transpose , the clasnotans for not transpose
      // the norm_date is 1 * (width * height)
      caffe_cpu_gemv<Dtype>(CblasTrans, channels, spatial_dim, Dtype(1),
                            buffer_data, sum_channel_multiplier, Dtype(1),
                            norm_data);
      // compute norm
      // norm = norm ** 0.5 (spatial == height * width)
      // for every pix (all channels) the norm
      caffe_powx<Dtype>(spatial_dim, norm_data, Dtype(0.5), norm_data);
      // scale the layer
      // buffer_data (wight * height) * (width * height) for every line 
      // large the buffer_data for the  next compute
